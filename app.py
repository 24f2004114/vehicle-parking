...
with app.app_context():
    db.create_all()
    if not Admin.query.first(): # initially database will not have username and password in database so we hardcoded it
    ...
    if request.method=="POST":
        ...
        if type=="location":
            try:
                ...
                return render_template('invalid.html',errormessage=e,back='user_dashboard')
        elif type=="pincode":
            try:
                ...
                return render_template('invalid.html',errormessage="pincode must be integer",back='user_dashboard')
    ...
    if request.method=="POST":
        try:
            ...
            return redirect(url_for("admin_dashboard"))
                ...
                return render_template('invalid.html',errormessage=e,back='admin_dashboard')
...
def admin():
    if request.method=="POST":
        ...
        if admin:
            ...
            return render_template("invalid.html", errormessage="Invalid credentials for admin ",back="home")
...
def registration():
    if request.method=="POST":
        try:
            ...
            if already_users:
                return render_template("invalid.html",errormessage="User already registered with this credentials",back="home")
            ...
            return redirect(url_for("home"))
                ...
                return render_template('invalid.html',errormessage=e,back='home')
...
def add_lot():
    if request.method=="POST":
        ...
        if already_exist:
            return render_template('invalid.html',errormessage="Parking lot already exist",back='admin_dashboard')
...
def view(lot_id,spot_id):
    ...
    if request.method=="POST":
        if not spot.Status:
            ...
            return render_template("invalid.html",errormessage="Can not delete an occupied parking lot",back='admin_dashboard')
    return render_template("view.html",spot=spot,booking=booking)

    # isko karna h pura

@app.route('/occupied_details/<int:booking_id>')
def occupied_details(booking_id):
    booking=Booking.query.filter_by(Booking_ID=booking_id).first()
    duration=((datetime.now()-booking.booking_time).total_seconds())/3600
    price_perhour=booking.spot.parkinglot.Price
    cost=round(duration*price_perhour,3)
    id=booking.spot_ID
